...
my $counters_conn		= "1.3.6.1.4.1.2620.1.16.23.1.1.2.1.0";	# "connNum" : number connections
my $counters_reject		= "1.3.6.1.4.1.2620.1.16.23.1.1.11.1.0";	# "rejected" : total rejections
my $counters_pack		= "1.3.6.1.4.1.2620.1.16.23.1.1.5.1.0";	# "rejected" : total rejections

my %counters_checks		= ($counters_conn,"total", $counters_reject,"rejects", $counters_pack,"packs");
my %counters_checks_n	= ($counters_conn,"Count total",$counters_reject,"rejects", $counters_pack,"packs");
my @counters_checks_oid	= ($counters_conn,$counters_reject,$counters_pack);
#################################### Globals ##############################""
...
my $o_count=	undef;		# Check for management status
my $o_policy=	undef;		# Check for policy name
my $o_conn=	undef;		# Check for connexions
my $o_perf=	undef;		# Performance data output 

# SNMPv3 specific
my $o_login='snmpuser';		# Login for snmpv3
my $o_passwd='MGNMY84T2eBhuEcuMVXFpdwjqpuRHXG';		# Pass for snmpv3
my $v3protocols=undef;	# V3 protocol list.
my $o_authproto='sha';		# Auth protocol
my $o_privproto='aes';		# Priv protocol
my $o_privpass= 'nkkeEXeozxxBBwNUqnkFrz3tNNjNJ6wD8He6aYVde23rjTZfD3';		# priv passwordAbb
...
sub check_options {
    ...
    if (defined($o_perf) && ! defined ($o_count))
	{ print "Nothing selected for cperfparse !\n";print_usage(); exit $ERRORS{"UNKNOWN"}}
...
# Check all states
  $resultat=undef;
  $resultat = $session->get_request(
      Varbindlist => \@counters_checks_oid
  );  
  verb("RE: " . $resultat);
  if (defined($resultat)) {
    foreach $key ( keys %counters_checks) {
      verb("C: $counters_checks_n{$key} : $counters_checks{$key} / $$resultat{$key}");
      $perf_counter .=  " $counters_checks{$key}:$$resultat{$key},";
    }
  } else {
    $counters_print .= "cannot find oids";
    #Critical state if not found because it means soft is not activated
    $counters_state=2;
  }  
  if ($counters_state == 0) {
    $counters_print="COUNTER : K";
  } else {
    $counters_print="COUNTER : " . $counters_print;
  }
  verb("$svn_print");
}
...
if (defined($o_perf) && defined ($perf_counter)) {
  chop($perf_counter);
  $f_print .= " | fw_counters=" . $perf_counter;
}
